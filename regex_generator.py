...
def main_regex_generator(path):
    ...
    ###### splitting the sent and the lable ######
    list_sent= list(data['sent'])
    list_labels = list(data['label'])
    ###### creating a list of lists with sentences and lable ######  
    mega_list_label=[]
    ...
    for i in mega_list_label:
        ...
        for i in re.findall(sub, str1, re.I):
            some_list.append ([" ".join([x for x in i if x != ""]),name])# some_list is a list of lists with the patter -> <word1><Label><word2>
